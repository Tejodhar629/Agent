...
const bhashini_service_1 = require("./bhashini.service");
const rag_service_1 = require("./rag.service");
const axios_1 = require("axios");
let ChatService = ChatService_1 = class ChatService {
    constructor(bhashiniService, ragService) {
        this.bhashiniService = bhashiniService;
        this.ragService = ragService;
        this.logger = new common_1.Logger(ChatService_1.name);
        this.openaiApiKey = process.env.OPENAI_API_KEY || '';
    }
    async processUserQuery(payload) {
        ...
        if (!originalQuery) {
            throw new common_1.BadRequestException('Request query parameter is empty. Please supply a valid query text or voice stream.');
        }
        ...
        const ragResult = await this.ragService.retrieveContext(englishQuery, stateScope, schemeFilterName);
        const englishDraft = await this.executeSynthesisAgent(englishQuery, ragResult.chunks, sanitizedQuery);
    ...
    async executeRouterAgent(query) {
...

{
  ...
  "requires_ocr": boolean
    ...
    async executeSynthesisAgent(query, chunks, unmaskedQuery) {
        if (chunks.length === 0) {
...
5. **PII Masking:** Never output raw personal identification details. If any user details are present in context, ensure they are masked.`
    ...
    enforceOutputGuardrails(draftResponse, chunks) {
    ...
    heuristicRouterFallback(query) {
        const lQuery = query.toLowerCase();
        let routing_path = 'GENERAL_HELP';
        let geographic_scope = null;
        let scheme_name = null;
        let requires_ocr = false;
        if (lQuery.includes('karnataka') || lQuery.includes('ssp'))
            geographic_scope = 'KARNATAKA';
        if (lQuery.includes('uttar pradesh') || lQuery.includes('unnao') || lQuery.includes('up'))
            geographic_scope = 'UTTAR PRADESH';
    ...
    heuristicSynthesisFallback(query, chunks) {
