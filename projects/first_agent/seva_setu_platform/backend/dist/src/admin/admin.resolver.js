"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AdminResolver = void 0;
const graphql_1 = require("@nestjs/graphql");
const common_1 = require("@nestjs/common");
const admin_service_1 = require("./admin.service");
const admin_types_1 = require("./admin.types");
const scheme_types_1 = require("../scheme/scheme.types");
const jwt_auth_guard_1 = require("../auth/jwt-auth.guard");
const current_user_decorator_1 = require("../auth/current-user.decorator");
let AdminResolver = class AdminResolver {
    constructor(adminService) {
        this.adminService = adminService;
    }
    async getPlatformMetrics(user) {
        const metrics = await this.adminService.getPlatformMetrics(user.sub, user.role);
        return {
            dailyActiveUsers: metrics.dailyActiveUsers,
            totalApplicationsTracked: metrics.totalApplicationsTracked,
            topSearchedSchemes: metrics.topSearchedSchemes.map(s => ({
                ...s,
                tags: s.tags || [],
            }))
        };
    }
    async updateScheme(user, id, input) {
        const s = await this.adminService.updateScheme(id, input, user.role);
        return {
            ...s,
            tags: s.tags || [],
        };
    }
};
exports.AdminResolver = AdminResolver;
__decorate([
    (0, graphql_1.Query)(() => admin_types_1.PlatformMetrics, { name: 'getPlatformMetrics' }),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], AdminResolver.prototype, "getPlatformMetrics", null);
__decorate([
    (0, graphql_1.Mutation)(() => scheme_types_1.Scheme, { name: 'updateScheme' }),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, graphql_1.Args)('id', { type: () => graphql_1.ID })),
    __param(2, (0, graphql_1.Args)('input')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, admin_types_1.SchemeInput]),
    __metadata("design:returntype", Promise)
], AdminResolver.prototype, "updateScheme", null);
exports.AdminResolver = AdminResolver = __decorate([
    (0, graphql_1.Resolver)(),
    __metadata("design:paramtypes", [admin_service_1.AdminService])
], AdminResolver);
//# sourceMappingURL=admin.resolver.js.map