...
export class SchemeService {
  ...
  // Reference to Prisma client to be dynamically imported/injected if available
  private prisma: any = null;

  // Hardcoded production-grade default welfare schemes used as direct fallbacks & seeds
  private readonly defaultSchemes = [
    ...
    },
    {
      ...
      dbtAmount: 14400.00, // Rs 1200 per month
      isActive: true
    },
    {
      ...
      dbtAmount: 4800.00, // Rs 400 per month baseline
      isActive: true
    },
    {
      ...
      dbtAmount: 100000.00, // Representative credit baseline
      isActive: true
    }
  ];

  constructor() {
    // Attempt dynamic resolution of Prisma client to avoid build errors if the module isn't loaded
    try {
  ...
  /**
   * Fetches all schemes from PostgreSQL database or returns whitelisted default lists as safety fallbacks.
   */
  async findAll(category?: string, stateScope?: string, search?: string): Promise<any[]> {
    ...
    return this.defaultSchemes.filter(scheme => {
      if (category && scheme.category !== category.toUpperCase()) return false;
      if (stateScope && scheme.stateScope !== 'CENTRAL' && scheme.stateScope !== stateScope.toUpperCase()) return false;
      if (search) {
  ...
  /**
   * Retreives a specific scheme by its primary ID.
   */
  async findOne(id: string): Promise<any> {
    ...
    if (!localScheme) {
      throw new NotFoundException(`Government scheme with ID "${id}" could not be located.`);
    }
    return localScheme;
  }

  /**
   * Evaluates dynamic eligibility criteria against a user profile using a rules engine.
   */
  async evaluateEligibility(schemeId: string, userProfile: UserProfileDto): Promise<SchemeMatchResult> {
    const scheme = await this.findOne(schemeId);
    const rules: EligibilityRules = scheme.eligibilityRules as EligibilityRules;
    ...
    // Rule 3: Annual Income ceiling bounds check
    ...
    // Rule 4: Gender mapping check
    if (rules.gender && rules.gender.length > 0) {
      totalCriteriaEvaluated++;
      const isGenderMatched = rules.gender.some(
        g => g.toUpperCase() === 'ALL' || g.toUpperCase() === userProfile.gender.toUpperCase()
      );
      if (!isGenderMatched) {
        isEligible = false;
    ...
    // Rule 5: Caste Category match
    if (rules.category && rules.category.length > 0) {
      totalCriteriaEvaluated++;
      const isCasteMatched = rules.category.some(
        c => c.toUpperCase() === userProfile.category.toUpperCase()
      );
      if (!isCasteMatched) {
        isEligible = false;
    ...
    // Rule 6: Academic Student Status
    ...
    // Rule 7: Physical Disability Status
    ...
    // Rule 8: MSME Business Ownership
    ...
    // Rule 9: Specific Occupational Roles
    if (rules.occupations && rules.occupations.length > 0) {
      totalCriteriaEvaluated++;
      const isOccupationMatched = rules.occupations.some(
        o => o.toLowerCase() === userProfile.occupation.toLowerCase()
      );
    ...
    // Rule 10: Geographic Scope State-Scope Match
  ...
  /**
   * Matches all available schemes against a dynamic user profile.
   * Feeds into the "Scheme Finder" engine.
   */
  async findMatchingSchemes(userProfile: UserProfileDto, category?: string): Promise<SchemeMatchResult[]> {
    ...
    // Sort results by eligibility (eligible first) and then by highest matching score descending
  ...
  /**
   * User Saved Schemes Interactions (Bookmarks)
   */
  async saveScheme(userId: string, schemeId: string): Promise<any> {
  ...
  async unsaveScheme(userId: string, schemeId: string): Promise<boolean> {
